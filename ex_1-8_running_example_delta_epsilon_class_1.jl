...
n = 4
p = 10
f_fun = [x -> (confidence(N(x[3:4])) - x[2] - x[7]), x -> (confidence(N(perturbation(x[3:6]))) - x[8]), x -> (x[1]^2  - x[5]^2 - x[9]), x -> (x[1]^2  - x[6]^2 - x[10])]
Df_fun = [x -> [0 -1 confidence(DN(x[3:4]))... 0  0 -1 0 0 0],
        x -> [0 0 confidence(DN(perturbation(x[3:6])) * gradient_perturbation(x[3:6]))... 0 -1 0 0],
        x -> [2*x[1] 0 0 0 -2*x[5] 0 0  0 -1 0],
        x -> [2*x[1] 0 0 0 0 -2*x[6] 0  0 0 -1]]
problem = Problem(f_fun, Df_fun, [[1, 3, 4], [2, 3, 4]])
qvs = [(Forall, 3), (Forall, 4), (Forall, 5), (Forall, 6)]
qcp = QuantifiedConstraintProblem(problem, qvs, [qvs, qvs, qvs, qvs], p, n)
X_0 = IntervalBox(interval(0, 1), interval(0,1))
X_0 = IntervalBox(interval(1/8, 1/8), interval(1,1))
ϵ_max = 1
p_in = [[interval(-1, 1)], [interval(-1,-1)], [interval(-ϵ_max, ϵ_max)], [interval(-ϵ_max, ϵ_max)]]
p_out = deepcopy(p_in)
G = [interval(minus_inf, -0.0001), interval(0.0001, plus_inf), interval(0, plus_inf), interval(0, plus_inf)]
