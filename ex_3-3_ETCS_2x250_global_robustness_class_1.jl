...
# display(DN(IntervalBox([0, 0, 0])))
# display(gradient_variables)
# display(gradient_perturbation([0, 0, 0, 0]))

n = 2
p = 6
f_fun = [x -> (confidence(N([x[1], x[2], x_r])) - x[5]), x -> (confidence(N([perturbation(x[1:4])..., x_r])) - x[6])]
Df_fun = [x -> [confidence(DN(IntervalBox([x[1], x[2], x_r])) * gradient_variables)... 0 0 -1 0],
        x -> [confidence(DN(IntervalBox([perturbation(x[1:4])..., x_r])) * gradient_variables * gradient_perturbation(x[1:4]))... 0 -1]]
...
ϵ_v = 0.05
ϵ_x_h = 1/50000
p_in = [[interval(-ϵ_v, ϵ_v)], [interval(-ϵ_x_h, ϵ_x_h)]]
p_out = deepcopy(p_in)
