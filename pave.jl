...
function create_is_out_1(qe::QuantifierProblem, intervals::AbstractVector{IntervalArithmetic.Interval{T}}, pseudo_infinity::Int=1000, ϵ::Float64=0.1)::Function where {T<:Number}
    return function(X::IntervalArithmetic.Interval{T}) where {T<:Number}
        ...
        dirty_quantifiers = quantifiedvariables2dirtyvariables(quantifiers)
        _, R_outer = QEapprox_o0(qe.f, qe.Df, dirty_quantifiers, [dirty_quantifiers for i=1:qe.n], qe.p, qe.n, [X, intervals...])
...
function make_paving_tree_from_constructors(constructors, intervals, is_member)
    ...
    child = make_paving_tree_from_constructors(intervals, root, constructors[2:end], is_member)
    add_child(root, child)
    return root
end

function make_paving_tree_from_constructors(intervals, parent::AbstractCell, constructors, is_member)
    if length(intervals) == 1
        return constructors[begin](intervals[begin], parent, is_member)
    else
        cell = constructors[begin](intervals[begin], parent)
        child = make_paving_tree_from_constructors(intervals[2:end], cell, constructors[2:end], is_member)
...
function bisect!(root::CellStart, qe, create_is_member)
    ...
    intervals_1 = [i == idx ? interval_1 : initial_intervals[i] for i in 1:(qe.p-1)]
    intervals_2 = [i == idx ? interval_2 : initial_intervals[i] for i in 1:(qe.p-1)]
